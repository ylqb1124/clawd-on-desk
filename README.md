...
- 🏷️ **状态标签** — 宠物下方实时显示中文状态说明（思考中..、编码中..、构建中.. 等），胶囊样式淡入淡出
...
| Claude 行为 | 颜色 | 表情 | 特效 | 标签 |
|-------------|------|------|------|------|
| 无会话 (sleeping) | 冷灰 | 闭眼横线 | Zzz 上浮 | 休眠 |
| 空闲 (idle) | 天蓝 | 圆眼 + 微笑 | 静止 | 空闲 |
| 思考 (thinking) | 琥珀橘黄 | 上看 + 短横嘴 | 身体微倾 | 思考中.. |
| 写代码 (typing) | 翠绿 | 眯眼 + 方块嘴 | 涟漪扩散 | 编码中.. |
| 编译 (building) | 珊瑚橙红 | 圆眼 + 微笑 | 齿轮旋转 | 构建中.. |
| 测试 (testing) | 青碧 | 圆眼 + 微笑 | 扫描线 | 测试中.. |
| 搜索 (searching) | 靛蓝 | 放大镜眼 | 雷达扫描 | 搜索中.. |
| 安装 (installing) | 紫罗兰 | 圆眼 + 微笑 | 进度环 | 安装中.. |
| 子代理 (subAgent) | 薄荷绿 | 圆眼 + 微笑 | 迷你分身环绕 | 子任务.. |
| 完成 (celebrate) | 金黄 | ^_^ 笑脸 | 弹跳 + 彩色烟花 | 完成！ |
| 出错 (error) | 正红 | ×_× 难过 | 摇晃 + 裂纹 | 出错了 |
| 需要审批 (attention) | 警示红 | 大眼 + ! | 脉冲红圈 | 需要授权 |
